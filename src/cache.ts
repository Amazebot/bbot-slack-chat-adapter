 ...
 */
export function use (instanceToUse: any): void {
...
export function call (method: string, key: string): Promise<any> {
  ...
  if (methodCache.has(key)) {
    bot.logger.debug(`[${method}] Calling (cached): ${key}`)
    ...
    bot.logger.debug(`[${method}] Calling (caching): ${key}`)
...
/** Proxy for set */
export function set (method: string, key: string, value: any) {
  if (!results.has(method)) create(method) // create on use
  return results.get(method)!.set(key, value)
}
